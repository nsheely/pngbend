//! The high-level public API: [`decode`] / [`decode_strict`] /
//! [`decode_with_events`] and [`encode`], plus the [`Image`] / [`GlassBox`] /
//! [`PngError`] types.
//!
...
//!
//! Both decode paths are safe by default: inflation is capped at the exact
//! output the IHDR dimensions call for, so a decompression-bomb PNG (a tiny
//! IDAT that would expand to gigabytes) is rejected rather than allocated. The
//! shared chunk-level preparation lives in `prep`, encode orchestration in
//! `encode`, the error type in `error`, and the result types in `image`.
...
pub use encode::{EncodeOptions, OutputFormat, encode};
