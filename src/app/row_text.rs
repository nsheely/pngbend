...
pub(super) fn append_row_text(out: &mut String, fref: FilterRef, row: &PixelRow, c: &CoreData) {
    ...
    let cluster_size = (x + c.raster.pixels_per_byte()).min(c.info.width) - x;
...
pub(super) fn lookup_ref_metrics(c: &CoreData, xy: PixelXY) -> Option<(usize, u16)> {
    let bpp = c.info.bpp;
    let base = c.raster.xy_to_out(xy).0 as usize;
    for ch in 0..bpp {
        let pos = base + ch;
